...
GameXMLFilesDirPath=[('L:/JMYsun/Arcadefile/Sega/Chunithm/SDBT_1.00-1.50/app/data/A000'),('L:\JMYsun\Arcadefile\Sega\Chunithm\SDBT_1.00-1.50\option\A001')]
SortFilesCD_Path=[(str(CD_Path)+'/SortFilesOutput/app/data/A000'),(str(CD_Path)+'/SortFilesOutput/option/A001')]
...
for i in range(2): 
    for j in range (5):
        ...
        SortFiles_Path=((str(SortFilesCD_Path[i]+'/'+XML_FileDirName[j]+'/'+XML_SortFileName[j]+'.xml')))
        SortFilesTXT_Path=((str(SortFilesCD_Path[i]+'/'+XML_FileDirName[j]+'/'+XML_SortFileName[j]+'.xml')))
